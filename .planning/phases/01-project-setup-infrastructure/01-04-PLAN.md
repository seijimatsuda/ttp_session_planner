...
<task type="auto">
  ...
  <action>
First, update CORS in backend to handle production origin. Update backend/src/app.ts:
...
render create --name soccer-session-planner-api --type web

# Or prepare for manual deployment
...
After deployment, note the Render URL (something like: https://soccer-session-planner-api.onrender.com)
  </action>
  <verify>
...
curl https://YOUR_RENDER_URL.onrender.com/api/test-db
```
  </verify>
  <done>Backend deployed to Render with health endpoint working</done>
</task>

<task type="checkpoint:human-action" gate="blocking">
  <name>Task 3: Deploy backend to Render (manual step)</name>
  <action>
User must deploy to Render using their account. The CLI approach may require auth.
  </action>
  <instructions>
...
<task type="checkpoint:human-action" gate="blocking">
  <name>Task 4: Deploy frontend to Vercel</name>
  <action>
...
5. Add Environment Variables (expand "Environment Variables"):
   ...
   - VITE_API_URL = [your Render backend URL from Task 3]
...
<task type="checkpoint:human-action" gate="blocking">
  <name>Task 5: Update CORS and verify end-to-end</name>
  <action>
User must update Render environment variable with frontend URL.
  </action>
  <instructions>
...
6. Test backend from frontend (in browser console):
   ```javascript
